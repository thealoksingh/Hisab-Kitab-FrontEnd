...
function FriendTranscationDetail({ user, selectedFriend }) {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const response = await getTransactionDetailsWithFriend(user.userId, selectedFriend.userId);
    ...
    <>
      ...
      <div className="right-header top-[0] h-[10%] bg-slate-400 w-full absolute p-[10px] flex gap-[10px] flex items-center ">
        <div className="alphabet-circle bg-teal-600 h-[50px] w-[50px] rounded-full flex items-center justify-center">
          <h1 className="text-white text-lg">{selectedFriend.fullName[0]}</h1>
        </div>
  ...
  className={`net-balance border h-[35px] w-40 rounded flex items-center justify-center ml-5 ${
    transactionsDto.length > 0 && transactionsDto[0].lastClosingBalance >= 0 ? 'border-teal-900 text-green-900' : 'border-red-900 text-red-900'
  }`}
>
  <h2 className={`text-black ${
    ...
    {transactionsDto.length > 0 && (
      <>
        {transactionsDto[0].lastClosingBalance >= 0 ? 'You will get :' : 'You will give :'}
  ...
  {transactionsDto.map((transactionDto, index) => {
    ...
    const isUserGave = user.userId === transactionDto.transaction.fromUserId;
    ...
    return (
      <tr
        ...
        onClick={() => handleRowClick(transactionDto)}
      >
        {/* Closing Balance */}
        <td
          scope="row"
        ...
        >
          <div className="flex flex-col">
            <span className="font-medium text-white">
              Closing Balance: <span>$</span> <span>{transactionDto.lastClosingBalance}</span>
            </span>
            <span className="text-xs text-white mt-1">{transactionDto.transaction.transDate}</span>
          </div>
        </td>
