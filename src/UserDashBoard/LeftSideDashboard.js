...
function LeftSideDashBoard({
    ...
    <>
      ...
      >
        ...
        <div className="left-upper sm:font-normal justify-between h-32   w-full">
         ...
         <div className="w-[32%] h-[100%] sm:h-[100%]   mt-3  sm:p-1  md:mt-0 pl-5  ">   
          {/* View Report Button */}
             <button
             onClick={handleDownload}
              className="bg-rose-600 text-white py-1 px-7 flex items-center justify-center hover:bg-rose-500 focus:outline-none focus:ring-4 focus:ring-rose-300 font-medium rounded-sm shadow-md transition-all duration-300 ease-in-out transform hover:scale-105"
               disabled={loading}
  >
             {/* Icon (Always Visible) */}
             <FontAwesomeIcon icon={faDownload} className="mr-2 py-1 right-0  " />
          ...
          <div className="text-sm  py-1 sm:py-3    bg-gray-300  flex h-32 relative gap-2 ">
            ...
            <div className="search-box w-[38%] ml-2 h-full mb-2  sm:mb-5">
              <p className="p-1 line-clamp-1  font-Poppins font-semibold  justify-center align-middle">Search Friend</p>
  ...
  <button
    ...
    className="relative w-[44%] h-full  flex items-center rounded-sm justify-center bg-green-700 text-sm text-white hover:bg-teal-700 focus:outline-none focus:ring-4 focus:ring-emerald-300 px-2 py-1 shadow-md transition-all duration-300 ease-in-out transform hover:scale-105"
  >
    {/* Notification Badge */}
    <div className="h-6 w-6 absolute -right-2 -top-2 border rounded-full border-white flex items-center justify-center bg-orange-500 text-white text-xs font-bold shadow-sm">
      {friendRequestCount}
    </div>
    View Friend Request
  </button>
