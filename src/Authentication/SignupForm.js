...
const SignUpForm = () => {
    ...
    if (validate()) {
      try {
        ...
        if(sendOTP.fulfilled.match(response)){
          ...
          console.log("OTP sent successfully in if:", response?.payload?.message);
        } else{
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-50 flex items-center bg-gray-300 bg-cover bg-center justify-center w-full h-screen  ${isOpen ? "" : "hidden"
        }`}
    >
